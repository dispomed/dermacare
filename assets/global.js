...
// create object class for handling push events
...
class Analytics {
  ...
  getPrice(price) {
    // Price logic
    if (price === undefined) {
      // get price from price-item .price-item--regular
      const priceEl = document.querySelector('span.price-item.price-item--regular');
      if (priceEl) {
        price = priceEl.textContent;
        // trim price, rm whitespaces, euro sign, and replace comma with . and EUR
        price = price.trim().replace(/\u20AC/g, '').replace(/,/g, '.').replace(/\s/g, '').replace(/EUR/g, '');
      }
    } else {
      price = price.replace(/\u20AC/g, '').replace(/,/g, '.').replace(/\s/g, '').replace(/EUR/g, '');
    }
    // convert to float
    price = parseFloat(price);

    // if price is NaN, raise error
    if (isNaN(price)) {
  ...
  getTitle(title) {
    if (title === undefined) {
      // get title el, .product__title > h1
      title = "";
      ...
      if (variantTitleEl) {
        title += " - " + variantTitleEl.textContent;
      }
    }
  ...
  getProductId(productId) {
    // get variant id from query string variant
    const urlParams = new URLSearchParams(window.location.search);
    const variantId = urlParams.get('variant');

    // if variant id is defined, return it and if not, return product id
    return variantId || productId;
  }

  addProductToCart(source, productId, price, title) {
    try {
  ...
  renderProductInfo() {
    ...
    fetch(
      ...
      .then((responseText) => {
        ...
        console.log('addbutton ', addButtonUpdated, price);
